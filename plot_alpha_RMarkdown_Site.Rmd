...
# Plot the data #
Finally, plot the soil moisture data using the `levelplot()` function from `rasterVis` for one month:
```{r plot_data}
# rasterVis plot
maptheme <- rasterTheme(region=brewer.pal(8,"Oranges"))
plt <- levelplot(soil, margin=F, par.settings=maptheme,main="Soil Moisture mean")
