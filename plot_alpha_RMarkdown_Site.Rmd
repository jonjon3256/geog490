...
+ Plotting raster before rotating coordinates 
```{r read_data}
...
soil_raster # Convert the coordinates of raster to -180-180 degrees
```

+ Plotting raster after rotating coordinates 
...
# Plot the data #
Finally, plot the soil moisture data using the `levelplot()` function from `rasterVis`:
```{r plot_data}
# rasterVis plot
